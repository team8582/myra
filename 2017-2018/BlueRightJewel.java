...
@Autonomous(name = "BlueRightJewel", group = "Autonomous")
//@Disabled
public class BlueRightJewel extends LinearOpMode {
    ...
    final double INIT_LEFT = 0.0;     //starting position of left servo
    final double INIT_RIGHT = 1.0;    //starting position of right servo
    ...
    public void runOpMode() throws InterruptedException {
        ...
        robot.colorSensor.enableLed(true); //turns on LED whose light causes jewels to reflect wavelengths the sensor can read
        ...
        if (isBlue) {
            strafeUsingRight(-39, 10); //strafe to the right to park in zone, knocking off red jewel
            robot.jewels.setPosition(UP_JEWELS);
            sleep(1000);
        
        //conditional statement for if the jewel to the left of the arm is red   
        } else if (isRed) {
            turnAbsolute(10); //turn enough to knock off red jewel
            sleep(1000);
            robot.jewels.setPosition(UP_JEWELS); //lift arm to avoid also knocking off blue jewel
            turnAbsolute(0); //return to starting position
            strafeUsingRight(-39, 10); //strafe until parked in front of cryptobox
        }
