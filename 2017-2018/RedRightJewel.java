...
@Autonomous(name = "RedRightJewel", group = "Autonomous")
//@Disabled
public class RedRightJewel extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        robot.colorSensor.enableLed(true); //turn on LED that causes the jewel to reflect wavelengths for the sensor to read
        ...
        robot.jewels.setPosition(DOWN_JEWELS); //lower servo arms
        sleep(2000);
        //variables for various color readings
        ...
        if (isRed) { //çonditional statement for if the jewel to the left of the arm is red
            turnAbsolute(-10); //turn enough to knock off blue jewel to the right of the arm
            sleep(1000);
            robot.jewels.setPosition(UP_JEWELS); //raise arm to avoid knocking red jewel while strafing
            turnAbsolute(0); //return to initial orientation
            strafeUsingRight(39); //strafe until parked in cryptobox zone

        } else if (isBlue) { //conditional statement for if the jewel to the left of the arm is blue
            strafeUsingRight(39); //strafe until parked in cryptobox zone
            robot.jewels.setPosition(UP_JEWELS); //raise arm
        }
